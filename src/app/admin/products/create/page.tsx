...
export default function CreateProductPage() {
  const [form, setForm] = useState({ name: '', description: '', category: 'Electronics', brand: '', price: '', discount: '0', stock: '', image_url: '', status: 'active' })
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              {parseFloat(form.price) > 0 && parseFloat(form.discount) > 0 && (
                <p className="text-green-600 text-xs mt-1">Final price: ₹{(parseFloat(form.price) - (parseFloat(form.price) * parseFloat(form.discount) / 100)).toFixed(2)}</p>
              )}
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                rows={4} placeholder="Product description..." className="w-full border border-gray-300 rounded-lg px-4 py-2.5 outline-none focus:border-green-500 text-sm resize-none" />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <Link href="/admin/products" className="flex-1 text-center border border-gray-300 text-gray-700 py-3 rounded-xl font-semibold hover:bg-gray-50 transition">Cancel</Link>
